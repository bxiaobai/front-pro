...
import { Button, Descriptions, Divider, Form, Modal, Table, TableProps, Tag } from "antd";
import DateTimeFormatter from "../../../../../../utils/Convent";

interface Props {
  visible: boolean;
  onCancel: () => void;
  onOk: (selectedDrugs: API.IrStrListVO[], patientInfo: API.Brxx) => void; // 修改 onOk 的签名
  patientInfo: API.Brxx;
  ...
  return (
    ...
    >
      <Descriptions bordered size="small">
        <Descriptions.Item label="姓名">{patientInfo?.brxm}</Descriptions.Item>
        <Descriptions.Item label="性别">{patientInfo?.brxb}</Descriptions.Item>
        <Descriptions.Item label="手机">{patientInfo?.lxdh}</Descriptions.Item>
        <Descriptions.Item label="卡号">{patientInfo?.blh}</Descriptions.Item>
        <Descriptions.Item label="患者类型">
          <Tag color={patientInfo?.brlx === 1 ? 'red' : 'blue'}>
            {patientInfo?.brlx === 1 ? '急诊' : '门诊'}
          </Tag>
        </Descriptions.Item>
      </Descriptions>
